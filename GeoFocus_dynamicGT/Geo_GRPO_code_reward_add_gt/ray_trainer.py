...
from verl.utils.checkpoint import CHECKPOINT_TRACKER, remove_obsolete_ckpt
from verl.utils.logger import Tracker
from verl.utils.py_functional import convert_dict_to_str
from verl.utils.seqlen_balancing import get_seqlen_balanced_partitions, log_seqlen_unbalance
from .fsdp import FSDPWorker
...
from .dataset import GeoRLHFDataset, collate_fn
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self) -> None:
        with open(self.config.data.system_prompt) as f:
            system_prompt = f.read()

        self.train_dataset = GeoRLHFDataset(
        ...
        # self.val_dataset = GeoRLHFDataset(
        ...
        #     max_prompt_length=self.config.data.max_prompt_length,
        #     truncation="right",
        #     system_prompt=r"You FIRST think about the reasoning process as an internal monologue and then provide the final answer. The reasoning process MUST BE enclosed within <think> </think> tags. The final answer MUST BE put in \boxed{}.",  # Enclose the model’s entire output within <answer></answer> tags.
        #     min_pixels=self.config.data.min_pixels,
        #     max_pixels=self.config.data.max_pixels,
        # )
        ...
        self.val_datasets = [
            GeoRLHFDataset(
                ...
                max_prompt_length=self.config.data.max_prompt_length,
                truncation="right",
                system_prompt=r"You FIRST think about the reasoning process as an internal monologue and then provide the final answer. The reasoning process MUST BE enclosed within <think> </think> tags. The final answer MUST BE put in \boxed{}.",
                # system_prompt="",
                min_pixels=self.config.data.min_pixels,
                max_pixels=self.config.data.max_pixels,
            )
            for path in self.config.data.val_files_list  # 假设这是一个 list
        ]
        ...
        self.val_dataloaders = [
            StatefulDataLoader(
                dataset=ds,
                batch_size=len(ds) if self.config.data.val_batch_size == -1 else self.config.data.val_batch_size,
                shuffle=False,
                num_workers=8,
                collate_fn=collate_fn,
                pin_memory=False,
        ...
        assert len(self.val_dataloaders) >= 1
        print(f"Size of train dataloader: {len(self.train_dataloader)}")
        print(f"Size of val dataloader: {len(self.val_dataloaders)}")
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for idx, val_dataloader in enumerate(self.val_dataloaders):
            ...
            for test_data in val_dataloader:
                ...
                test_output_gen_batch = unpad_dataproto(test_output_gen_batch, pad_size=pad_size * test_gen_batch.meta_info['n'])
                ...
                # modified here to support average pass@1 validation
                if test_gen_batch.meta_info['n'] > 1:
                    test_batch = test_batch.repeat(repeat_times=test_gen_batch.meta_info['n'], interleave=True)
            ...
            val_reward_metrics = {f"val{idx}/{key}_reward": value for key, value in reduce_metrics(reward_metrics_lst).items()}
            
            all_val_metrics[f"val{idx}/reward_score"] = reward_score
            all_val_metrics.update(val_reward_metrics)

         # --- TensorBoard Logging Section ---
        accuracy_rewards = []

        for idx in range(len(self.val_dataloaders)):
            key = f"val{idx}/accuracy_reward"
            if key in all_val_metrics:
                acc = all_val_metrics[key]
                accuracy_rewards.append(acc)

        if accuracy_rewards:
            avg_acc_reward = sum(accuracy_rewards) / len(accuracy_rewards)
            all_val_metrics["val/avg_accuracy_reward"] = avg_acc_reward
    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                ground_truth_copy = batch.non_tensor_batch["ground_truth"].copy()

                # pop those keys for generation
                if "multi_modal_inputs" in batch.non_tensor_batch.keys():
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "multi_modal_inputs", "ground_truth"],
                    )
                else:
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["raw_prompt_ids", "ground_truth"],
                    )

                with _timer("step", timing_raw):
                    # generate a batch
                    with _timer("gen", timing_raw):  # wg: worker group
                        gen_batch_output = self.actor_rollout_wg.generate_sequences_mss(gen_batch)
                    ...
                    batch.non_tensor_batch["ground_truth"] = ground_truth_copy
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        elapsed_reward = time.time() - start_time_reward
                        mem_usage = process.memory_info().rss / (1024 ** 2)
                        logger.info(f"[Epoch] Time: {elapsed_reward:.2f}s | Memory: {mem_usage:.2f}MB")
                        gc.collect()
