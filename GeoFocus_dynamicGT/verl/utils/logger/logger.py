...
class TensorBoardLogger(Logger):
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # config = self.hparams_filter(flatten_dict(config))
        # self.writer.add_hparams(config,{})

    def hparams_filter(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """""value should be one of int, float, str, bool, or torch.Tensor"""
        return {k: v for k, v in data.items() if isinstance(v, (int, float, str, bool, torch.Tensor))}
...
class Tracker:
    def __init__(self, loggers: Union[str, List[str]] = "console", config: Optional[Dict[str, Any]] = None):
        ...
        if "tensorboard" in loggers:
            writer = self.loggers[loggers.index("tensorboard")].writer
            
        # self.gen_logger = AggregateGenerationsLogger(loggers)
        self.gen_logger = AggregateGenerationsLogger(loggers, tensorboard_writer=writer if "tensorboard" in loggers else None)
