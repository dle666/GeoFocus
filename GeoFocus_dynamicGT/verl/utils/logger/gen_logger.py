...
@dataclass
class TensorBoardGenerationLogger(GenerationLogger):
    ...
    def log(self, samples: List[Tuple[str, str, float]], step: int) -> None:
        for i, sample in enumerate(samples):
            row_text = f"input: {sample[0]}\n\n---\n\noutput: {sample[1]}\n\n---\n\nscore: {sample[2]}"
            row_text = re.sub(r'<', '&lt;', re.sub(r'>', '&gt;', row_text))
            self.writer.add_text(f"val/generations/sample_{i + 1}", row_text, step)

GEN_LOGGERS = {
    "console": ConsoleGenerationLogger,
    "wandb": WandbGenerationLogger,
    "swanlab": SwanlabGenerationLogger,
    "tensorboard": TensorBoardGenerationLogger,
}


@dataclass
class AggregateGenerationsLogger:
    def __init__(self, loggers: List[str], tensorboard_writer: SummaryWriter = None):
        self.loggers: List[GenerationLogger] = []

        for logger in loggers:
            if logger in GEN_LOGGERS and logger != "tensorboard":
                self.loggers.append(GEN_LOGGERS[logger]())
            elif logger == "tensorboard" and tensorboard_writer is not None:
                self.loggers.append(GEN_LOGGERS[logger](tensorboard_writer))
            else:
