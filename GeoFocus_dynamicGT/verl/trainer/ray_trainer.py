...
class RayPPOTrainer:
    ...
    def _create_dataloader(self) -> None:
        ...
        self.val_datasets = [
            ...
            for path in self.config.data.val_files_list 
        ]
    ...
    def fit(self):
        ...
        # load checkpoint before doing anything 此处没有任何操作
        self._load_checkpoint()  # self.config.trainer.load_checkpoint_path is None
        ...
        for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == "remax":  # 没进
                    ...
                    # 为批次中的每个样本分配一个全局唯一ID（UUID），用于后续跟踪、日志记录或数据关联。
                    batch.non_tensor_batch["uid"] = np.array(
                        [str(uuid.uuid4()) for _ in range(len(batch.batch))], dtype=object
                    )

                    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    print(gen_batch_output.batch["input_ids"].shape, gen_batch_output.batch["attention_mask"].shape)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if not self.config.algorithm.use_kl_loss and self.use_reference_policy:  # apply kl penalty to reward 没进
